...
unsigned int nStakeTargetSpacing = 18; // 18 seconds
...
const string strMessageMagic = "X2 Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
    ...
    else if (pindexBest->nHeight <= 64)
      {
        int64_t nSubsidy = 32 * COIN;
        return nSubsidy + nFees;
      }      

    else if (pindexBest->nHeight <= 128)
      {
        int64_t nSubsidy = 64 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 256)
      {
        int64_t nSubsidy = 128 * COIN;
        return nSubsidy + nFees;
      }   

	      else if (pindexBest->nHeight <= 512)
      {
        int64_t nSubsidy = 256 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 1024)
      {
        int64_t nSubsidy = 512 * COIN;
        return nSubsidy + nFees;
      }   

    else if (pindexBest->nHeight <= 2048)
      {
        int64_t nSubsidy = 1024 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 4096)
      {
        int64_t nSubsidy = 2048 * COIN;
        return nSubsidy + nFees;
      }   	  
 
    else if (pindexBest->nHeight <= 8192)
      {
        int64_t nSubsidy = 4096 * COIN;
        return nSubsidy + nFees;
      }   	   
      
    else if (pindexBest->nHeight <= 16384)
      {
        int64_t nSubsidy = 8192 * COIN;
        return nSubsidy + nFees;
      }   	  
 
    else if (pindexBest->nHeight <= 32768)
      {
        int64_t nSubsidy = 16384 * COIN;
        return nSubsidy + nFees;
      }   	   
...
{  
    ...
    if(pindexBest->nHeight <= 800000)
    {
        nSubsidy = 2000 * COIN; 
		return nSubsidy + nFees; 
    }

		else if(pindexBest->nHeight <= 950000)
	{
        nSubsidy = 1000 * COIN; 
		return nSubsidy + nFees; 
	}	
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "X2", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
